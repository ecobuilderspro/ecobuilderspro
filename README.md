# EcoBuilders site — v1.1

Static site served by GitHub Pages. Everything here goes at the repo root.

## What's in here

| File | What it is |
|---|---|
| `index.html` | Home page |
| `services/*.html` | Six service pages |
| `styles.css` | All styling for every page |
| `site.js` | Theme toggle, mobile nav, phone reveal, gallery |
| `assets/` | Photos, logos, badges |
| `build.py` | Generates the HTML. Optional — see below |

## Custom domain — read this before replacing files

The site is served at **buildecoenergy.com**. GitHub keeps a file named `CNAME`
at the repo root holding that domain. **Do not delete it.** If you wipe and
replace all the files again, put `CNAME` back or the custom domain drops and
the site reverts to the github.io URL.

Cloudflare DNS for reference — all records must be **DNS only (grey cloud)**,
not proxied, or GitHub cannot issue its certificate:

```
A     @     185.199.108.153
A     @     185.199.109.153
A     @     185.199.110.153
A     @     185.199.111.153
CNAME www   ecobuilderspro.github.io
```

## Installing

Replace the old files with these. Then **delete** the following from the repo,
they are no longer used:

```
gallery.js
assets/cubix-megapack-crane.jpg      assets/kaiser-diamond-bar.jpg
assets/getty-center.jpg              assets/ups-fleet-depot.jpg
assets/domaine-carneros-solar.jpg    assets/domaine-ground-mount.jpg
assets/sanford-aerial.jpg            assets/edge-ems.jpg
assets/commissioning-rope-ct.jpg     assets/om-ev-charging.jpg
assets/om-iv-tracer.png              assets/solar-ground.jpg
assets/solar-rooftop.jpg             assets/functional-testing.jpg
```

Those were renamed to neutral filenames — the old ones named clients in the
URL — or dropped because nothing used them.

## Adding a project later

All project content lives in one place: the `P = { ... }` block near the top of
`build.py`. One entry per project:

```python
 "mf-somewhere": dict(
    title="12 Pedestals · Somewhere, CA",       # what it is + city
    specs="12 pedestals · Level 2",             # the numbers
    note="One line about the scope.",
    images=[("multifamily-somewhere.jpg", "Caption for this photo."),
            ("multifamily-somewhere-2.jpg", "Caption for the second photo.")]),
```

Then add the key to the right section in `SERVICES` further down, drop the
photos in `assets/`, and run:

```
python3 build.py
```

That rewrites `index.html` and all six service pages. Commit the result.

A project key can appear on more than one service page — that is how one job
shows up as evidence in several places. The Napa winery carries solar, storage,
controls and commissioning; the insulation testing entry appears on both
commissioning and O&M.

If you'd rather not run Python, the generated HTML is perfectly editable
directly — just remember a change to the header has to be made in all seven files.

## Writing captions

- `title` — what it is plus the city. No client, manufacturer or network names.
- `specs` — the numbers. Capitalize the first segment, lowercase after each `·`
  unless it is a proper term (Level 2, DC fast, QA/QC, Modbus, OCPP).
- Don't publish a count you aren't sure of. Leave it out instead.
- Where a photo stands in for a site you don't have a picture of, keep the
  caption describing the real job and swap the file later.

## Theme

Colours are defined once as tokens at the top of `styles.css` — `--bg`,
`--surface`, `--text`, `--muted`, `--border`. Dark mode re-declares the same
names under `[data-theme="dark"]`. Change a colour there and it changes
everywhere, in both themes.

The theme follows the visitor's system setting until they click the sun/moon
button, after which their choice is remembered. A small script in each page's
`<head>` sets it before the page paints, so dark-mode visitors never see a
white flash.

## Photos

Images are resized to 1600px wide and compressed. If you drop in a new one
straight off a phone, give it the same treatment or the page gets slow.

## Email

Contact address across the site is `info@buildecoenergy.com`, appearing 30
times across the seven pages. It is set in three places in `build.py` — the
`MAIL` constant, the footer partial, and the contact card — so changing it is
a three-line edit plus a rebuild, never a hand-edit of the HTML.

## Still to do

- Photos for San Francisco, Bay Point, Camden NJ, Canoga Park, and the JPI
  Riverside site — those five projects are parked, not on the site.
- A favicon (browsers request one and currently get a 404).
- Crops flagged during review: truck-door logo on the corridor drone shot,
  charger-face lettering on the Torrance photo, fuel-canopy branding at
  Chula Vista, building signage on the Irwindale aerial.
- The LEED angle is unused — Long Beach is LEED ND Gold, San Francisco is
  net-zero / GreenPoint Platinum, and you're LEED AP. Nothing connects them yet.
